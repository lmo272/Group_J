...
class DataHandler:
    ...
    forecast(self, month: int):
        ...
        corresponding to the average rental with a shaded area corresponding to an
        interval of [-1 std deviation, +1 std deviation.].
    """
    ...
    def plot_correlation_matrix(self):
        ...
        sns.heatmap(
            ...
            annot=True)
        plt.title(
            "Correlation between month, weathersituation, temperature, windspeed and bike rentals",
    ...
    def plot_monthly_average(self):
        ...
        plt.bar(x=range(1, 13),
                height=self.dataframe
                .groupby(self.dataframe.index.month).sum("cnt")["cnt"].values * 0.5)
    ...
    def forecast(self, month: int):
        ...
        Returns
        ---------
        A plot with two visual elements. The plot has a 168h period on the x-axis
        and the number of bikes rented on the y-axis.

        The first visual element is the blue line,
        showing the expected rentals for that week based on the
        average rentals of the two years in the dataset.

        The second visual elements is the gray shayed area,
        representing the expected rental for the week
        ...
        axis.fill_between(df_month.index,
                          df_month['mean'] - df_month['std'],
                          df_month['mean'] + df_month['std'],
                          alpha=0.4)

        df_month['mean'].plot(ax=axis)

        axis.set_ylabel("# of bike rentals", fontsize=16)
        axis.set_xlabel("hour", fontsize=16)
